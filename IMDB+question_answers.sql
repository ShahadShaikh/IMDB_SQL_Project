...
SELECT 
    genre, COUNT(movie_id) AS no_of_movies_prod
FROM
    genre AS g
        INNER JOIN
    movie AS m ON g.movie_id = m.id
GROUP BY genre
ORDER BY no_of_movies_prod DESC
LIMIT 1;
...
SELECT 
    COUNT(movie_id) AS movie_count, median_rating
FROM
    ratings AS r
        INNER JOIN
...
top_genre_summary
WHERE g.genre in (top_genre_summary.genre) AND avg_rating>8
GROUP BY n.name
ORDER BY movie_count DESC
)
